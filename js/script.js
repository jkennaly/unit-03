...
const getSelectedOption = $select => $select.children()[$select.prop('selectedIndex')]
...
const optionsByPunniness = $select => {
	...
	const $punny = $options.filter((i, option) => punsSelected(option.innerHTML))
	const $notPunny = $options.filter((i, option) => !punsSelected(option.innerHTML))
	return {
		$punny: $punny,
		$notPunny: $notPunny
	}
...
const punnyInit = punsSelected(
	getSelectedOption($('#design'))
)
...
const calcRunningTotal = $checkedActivities => $checkedActivities
	.toArray()
...
const $initCheckedActivities = $activities
	.filter((i, activity) => activity.checked)
...
$activities.on('change', e => {
	...
	const $uncheckedActivities = $activities
		.filter((i, activity) => !activity.checked)
	...
	const selectedTimes = $checkedActivities.toArray()
	...
	const $disabledActivities = $uncheckedActivities
		.filter((i, activity) => selectedTimes.indexOf(getTimeFromInput(activity)) > -1)
		.prop('disabled', true)

	//enable each unchecked item with a time that does not match selectedTimes
	const $enabledActivities = $uncheckedActivities
		.filter((i, activity) => selectedTimes.indexOf(getTimeFromInput(activity)) < 0)
		.prop('disabled', false)
...
const showHidePaymentDivs = (showIndex, $paymentDivs) => $paymentDivs.each((index, el) => {
...
const $paymentDivs = $('#credit-card').add(
...
const errorMessageEl = message => $('<span />').addClass('error').html(message)

const attachErrorMessage = ($expectingParent, message) => $expectingParent
	.append(errorMessageEl(message))
...
const emailFieldInvalidMessage = 'Email format must be similar to "dave@teamtreehouse.com"'



//no activity
const noActivitySelected = () => !$activities
		.filter((i, activity) => activity.checked)
		.length
...
$form.on('submit', e => {
	...
	const errorPresent = errors.reduce((err, possibleErr) => err || possibleErr, false)
